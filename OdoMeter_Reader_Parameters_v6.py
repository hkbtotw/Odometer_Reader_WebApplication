...
# Fix Orientation from reading in iamge with PIL (cv doesnot need it, but cv not work on azure web app)
...
def Detect_Meter_PIL(img_path,ocr_url,headers):
    

    original = Image.open(img_path, mode="r")

    original=_fix_image_rotation(original)

    # Grey scale
    im2 = original.convert("L")

    # Denoise  
    im_arr=np.asarray(im2)
    im_med = median_filter(im_arr, 3)

    imgByteArr = io.BytesIO()
    pil_im = Image.fromarray(im_med)

    pil_im.save(imgByteArr, format='PNG')
...
def Number_Reader_ReadAPI_3_PIL(image, ocr_url, subscription, meterType):
    ...
    # Denoise
    im_arr=np.asarray(im3)
    im_med = median_filter(im_arr, 3)

    # Rescale
    imgin=rescale_image(im_med)


    #Convert back to PIL to transform to Byte
    pil_im = Image.fromarray(imgin)
    pil_im.save(imgByteArr, format='PNG')
    byte_im= imgByteArr.getvalue()
...
def Number_Reader_ReadAPI_3_PIL_BINARY(image, ocr_url, subscription, meterType):
    ...
    threshold = 125
    im = pil_im.point(lambda p: p > threshold and 255)
